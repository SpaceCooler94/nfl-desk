...
function propsFor(card, gid) {
  return (card.props || []).filter((p) => p.game_id === gid)
...
function sheetCSS() {
  return ":root{--bg:#0a0c10;--ink:#f3f5f8;--mute:#8b93a2;--pink:#ff2d7b;--good:#3DDC97;--bad:#e35d6a}*{box-sizing:border-box}html,body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.35 -apple-system,system-ui}.wrap{max-width:920px;margin:0 auto;padding:16px 14px 48px}.kicker{display:flex;justify-content:space-between;align-items:center}.kicker-l{font-size:11px;letter-spacing:.14em;color:var(--mute);font-weight:700}.kicker-l b{display:inline-block;background:#1c212b;color:#fff;padding:3px 8px;border-radius:4px;margin-right:8px}.logo{font-weight:900;font-size:26px}.logo span{color:var(--pink)}h1{font-size:30px;line-height:1;margin:8px 0 6px;font-weight:800}.meta{color:var(--mute);font-size:11px;letter-spacing:.08em;text-transform:uppercase;margin-bottom:12px}.teams{display:grid;grid-template-columns:1fr 42px 1fr;border-radius:12px;overflow:hidden;margin-bottom:10px}.away{background:linear-gradient(90deg,#0a3d44,#0e5560);padding:16px 14px}.home{background:linear-gradient(90deg,#9a3a12,#d2551a);padding:16px 14px;text-align:right}.lab{font-size:10px;letter-spacing:.16em;opacity:.75}.nm{font-size:20px;font-weight:800;margin-top:2px}.imp{font-size:11px;opacity:.88;margin-top:2px}.at{display:grid;place-items:center;font-weight:800;background:#0a0c10}.pills{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:0 0 12px}.pill{background:#12151c;border:1px solid #232833;border-radius:10px;padding:11px 6px;text-align:center}.pill .k{font-size:9px;color:var(--mute);letter-spacing:.14em;font-weight:700}.pill .v{font-size:15px;font-weight:800;margin-top:4px}.panel{background:#12151c;border:1px solid #232833;border-radius:12px;padding:12px 12px 6px;margin-bottom:10px}.ph{display:flex;justify-content:space-between;align-items:center;margin:2px 0 8px}.ph b{font-size:12px;letter-spacing:.1em;border-left:3px solid var(--pink);padding-left:8px}.ph span{font-size:10px;color:var(--mute)}table{width:100%;border-collapse:collapse}th{text-align:left;font-size:10px;color:var(--mute);letter-spacing:.12em;padding:6px 4px}th.r,td.r{text-align:right}td{padding:11px 4px;border-top:1px solid #232833}.who{display:flex;gap:10px;align-items:center}.dot{width:22px;height:22px;border-radius:50%;background:#1d232d;display:grid;place-items:center;font-size:8px;font-weight:800}.pn{font-weight:700;font-size:15px}.ps{font-size:11px;color:var(--mute)}.pos{color:var(--good);font-weight:700}.neg{color:var(--bad);font-weight:700}.grade{font-weight:800;color:var(--pink);font-size:16px}.cols{display:grid;grid-template-columns:1fr 1fr;gap:8px}.col h3{font-size:11px;color:var(--mute);margin:0 0 6px}.mrow{display:flex;justify-content:space-between;align-items:center;padding:9px 2px;border-top:1px solid #232833}.badge{min-width:34px;text-align:center;font-weight:800;padding:4px 6px;border-radius:6px}.g{background:#16361f;color:#3DDC97}.y{background:#3a3210;color:#e8c547}.r{background:#3a1518;color:#ff6b6b}.foot{display:flex;justify-content:space-between;color:var(--mute);font-size:11px;margin-top:8px}.foot b{color:var(--pink)}";
}
...
function sheetHTML(card, g) {
  ...
  const body = rows.map((p) => {
    ...
    return "<tr><td><div class=\"who\"><div class=\"dot\">" + esc(p.team || "") +
  ...
  return "<!DOCTYPE html><html><head><meta charset=\"utf-8\"/><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"/><style>" +
    ...
    esc(week) + "</b> " + sport + " · GAME SHEET</div><div class=\"logo\"><span>x</span>DESK</div></div><h1>" +
    esc(nameOf(g.away)) + " AT " + esc(nameOf(g.home)) + "</h1><div class=\"meta\">" + esc(whenLine) +
    "</div><div class=\"teams\"><div class=\"away\"><div class=\"lab\">AWAY</div><div class=\"nm\">" +
    esc(nameOf(g.away)) + "</div><div class=\"imp\">" + esc(ai) + "</div></div><div class=\"at\">AT</div><div class=\"home\"><div class=\"lab\">HOME</div><div class=\"nm\">" +
    esc(nameOf(g.home)) + "</div><div class=\"imp\">" + esc(hi) + "</div></div></div><div class=\"pills\">" +
    pillHTML + "</div><div class=\"panel\"><div class=\"ph\"><b>PROPS AT A GLANCE</b><span>PROJ VS LINE</span></div><table><thead><tr><th>PLAYER</th><th class=\"r\">PROJ</th><th class=\"r\">LINE</th><th class=\"r\">GAP</th><th class=\"r\">GRADE</th></tr></thead><tbody>" +
    body + "</tbody></table></div><div class=\"panel\"><div class=\"ph\"><b>KEY MATCHUPS</b><span>SCORE 0 TO 100</span></div><div class=\"cols\">" +
    lookCol((g.home || "") + " OFFENSE VS " + (g.away || ""), homeLooks) + lookCol((g.away || "") + " OFFENSE VS " + (g.home || ""), awayLooks) +
